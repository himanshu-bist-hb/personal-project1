...
class Auto:
    def __init__(self, StateAbb, State, rateTables, nEffective, rEffective, NGICRatebook, NAFFRatebook, NACORatebook, NICOFRatebook, NWAGRatebook, MMRatebook,NAICSDescriptions, SchedRatingMod) -> None:
        ...
        self.no_med_states = pd.read_excel("BA input File.xlsx", sheet_name="No MedPay")
        self.pip_states = pd.read_excel("BA input File.xlsx", sheet_name="PIP States")
        self.createMM()
    ...
    def process_ratebook(self, name, rateTables):
        ...
        LCM_sheet_mapping = pd.read_excel("BA Input File.xlsx", sheet_name="LCM-Comp Dev Mapping")
        LCM_map = pd.read_excel("BA Input File.xlsx", sheet_name="LCM-Map")
    ...
    # This is an old version of compareCompanies that only supports minimal comparison. Kept here as of 1/15/2026 incase something fails with the new one
    # If more than 1-2 months has passed without its use, you can delete this block.
    # def compareCompanies(self, tableCode):
    #     """
    #     Comparison method checks whether two tables are equal for companies that exist and creates clusters based on those comparisons.
    #     Then stores this info for self.buildBAPages method to use.
    #     -- Be very careful when editting. This is a core proccess of the program.
    #     """
    #
    #     CompaniesCheck = ['NGIC', 'NACO', 'NAFF','CCMIC','HICNJ', 'NICOF', 'NMIC', 'AICOA', 'NICOA', 'NPCIC']
    #
    #     self.default_company = ["NGIC"] # Used for comparisons when generating the sheets.
    #     self.CompanyListDif = []
    #     self.RemainingCompanies = ""
    #
    #     def compare_tables(company1, company2, tableCode):
    #         table1 = pd.DataFrame(data=self.rateTables[company1][tableCode])
    #         table2 = pd.DataFrame(data=self.rateTables[company2][tableCode])
    #
    #         table1 = table1.sort_values(by=table1.columns.values.tolist()).dropna().reset_index(drop=True)
    #         table2 = table2.sort_values(by=table2.columns.values.tolist()).dropna().reset_index(drop=True)
    #
    #         table1 = table1.applymap(lambda x: round(x, 3) if isinstance(x, (int, float)) else x)
    #         table2 = table2.applymap(lambda x: round(x, 3) if isinstance(x, (int, float)) else x)
    #
    #         # This is to deal with floating precision errors.
    ...
    #
    #     self.existing_companies = [company for company in CompaniesCheck if company in self.rateTables and self.rateTables[company] is not None]
    #
    #     if self.default_company[0] not in self.existing_companies and "NMIC" in self.existing_companies: # NGIC does not exist use NMIC as base instead.
    #         self.default_company = ["NMIC"]
    #     elif self.default_company[0] not in self.existing_companies and "CCMIC" in self.existing_companies:
    ...
    #             if compare_tables(company, representative,tableCode):
    ...
    def buildBaseRates(self, company, rate_type : str):
        ...
        state_mapper = pd.read_excel("BA Input File.xlsx", sheet_name=sheet_name)
        ...
        output_table = output_table.applymap(lambda x: "NA" if isinstance(x, (int, float)) and (abs(x) < 1e-6) else x)
        output_table = output_table.applymap(lambda x: "NA" if x == "0.00" else x)
    ...
    def buildTTTNAICSFactors(self, company):
        ...
        NAICSCoverages.iloc[:, 1:] = NAICSCoverages.iloc[:, 1:].astype(float).applymap(lambda x: f"{x:.3f}")        #NAICSCoverages = self.buildDataFrame("NAICSFactors_Ext").pivot(index='NAICSCode', columns='VehicleAndCoverageType', values='Factor').reset_index('NAICSCode').rename(columns={'NAICSCode' : 'NAICS Six-Digit Code'}).filter(items=['NAICS Six-Digit Code', 'NAICS Category', 'Trucks, Tractors, And Trailers Liability', 'Trucks, Tractors, And Trailers Comprehensive And Specified Causes Of Loss', 'Trucks And Truck-tractors Collision', 'Trailers Collision']).astype({'NAICS Six-Digit Code': 'int64'})
    ...
    def buildFleetSizeRatingFactorsZone(self,company):
        ...
        table.iloc[:,1:] = table.iloc[:,1:].astype(float).applymap(lambda x: f"{x:.3f}")
    ...
    def buildZoneSecondaryFactors(self, company):
        ...
        SecondaryFactors = SecondaryFactors.sort_values('4th-5th Digits of Class Code')
        SecondaryFactors.iloc[:, 2:] = SecondaryFactors.iloc[:, 2:].astype(float).applymap(lambda x: f"{x:.3f}")
    ...
    def build231C(self, company):
        ...
        if self.StateAbb == "NC":
            ...
            NC_rule231_table.iloc[:, 1:] = NC_rule231_table.iloc[:, 1:].astype(float).applymap(lambda x: f"{x:.3f}")
    ...
    def buildLayupFactors(self, company):
        ...
        LayupFactors.iloc[:, 1:] = LayupFactors.iloc[:, 1:].astype(float).applymap(lambda x: f"{x:.3f}")
    ...
    def buildTowingAndLabor(self, company):
        ...
        TowingAndLabor.iloc[:, 1:] = TowingAndLabor.iloc[:, 1:].applymap(lambda x: f"{x:.2f}")
    ...
    def buildPublicAutoLiabFleetSizeFactor(self, company):
        ...
        output_table.iloc[:,1:] = output_table.iloc[:,1:].astype(float).applymap(lambda x: f"{x:.3f}")
    ...
    def buildGarageKeepers1(self, company):
        ...
        GKFactors.iloc[:, 1:] = GKFactors.iloc[:, 1:].astype(float).applymap(lambda x: f"{x:.0f}")
    ...
    def buildGarageKeepers3(self, company):
        ...
        GKColl.iloc[:, 1:] = GKColl.iloc[:, 1:].astype(float).applymap(lambda x: f"{x:.3f}")
    ...
    def buildRule68table1(self, company):
        ...
        Rule68Table2.iloc[:, 1:] = Rule68Table2.iloc[:, 1:].astype(float).applymap(lambda x: f"{x:.3f}")

        no_med = pd.read_excel("BA input File.xlsx", sheet_name="No MedPay")
    ...
    def buildRule68table3(self, company):
        ...
        Rule68Table3.iloc[:,:] = Rule68Table3.iloc[:,:].astype(float).applymap(lambda x: f"{x:.3f}")
    ...
    def buildRule68table4(self, company):
        ...
        Rule68Table4.iloc[:, 1:] = Rule68Table4.iloc[:, 1:].astype(float).applymap(lambda x: f"{x:.3f}")
    ...
    def buildRule79table2(self, company):
        ...
        helper_map = pd.read_excel("BA Input File.xlsx", sheet_name="283 Helper Map")
    ...
    def buildRule279table3(self, company):
        ...
        output_table.iloc[:,1:] = output_table.iloc[:,1:].applymap(lambda x: "${:,.2f}".format(x))
    ...
    def buildRule83table1(self, company):
        ...
        Rule83Table1a.iloc[:, 1:] = Rule83Table1a.iloc[:, 1:].astype(float).applymap(lambda x: f"{x:.3f}")
    ...
    def buildRule83table2(self, company):
        ...
        Rule83Table2a.iloc[:, 1:] = Rule83Table2a.iloc[:, 1:].astype(float).applymap(lambda x: f"{x:.3f}")
    ...
    def buildRule83table3(self, company):
        ...
        Rule83Table3a.iloc[:, 1:] = Rule83Table3a.iloc[:, 1:].astype(float).applymap(lambda x: f"{x:.3f}")
    ...
    def buildRule83table4(self, company):
        ...
        output_table.iloc[:,1:] = output_table.iloc[:,1:].applymap(lambda x: f"{x:.2f}")
    ...
    def buildRule89table1(self, company):
        ...
        Rule89Table1.iloc[:,1:] = Rule89Table1.iloc[:,1:].applymap(lambda x: f"{x:.0f}")
    ...
    def buildRule89table2(self, company):
        ...
        Rule89Table2.iloc[:,:] = Rule89Table2.iloc[:,:].astype(float).applymap(lambda x: f"{x:.3f}")
        return Rule89Table2
    ...
    def buildRule89table3(self, company):
        ...
        Rule89Table3.iloc[:,:] = Rule89Table3.iloc[:,:].astype(float).applymap(lambda x: f"{x:.3f}")
    ...
    def buildRule89table4(self, company):
        ...
        Rule89Table4.iloc[:,:] = Rule89Table4.iloc[:,:].astype(float).applymap(lambda x: f"{x:.3f}")
    ...
    def buildRule89table5(self, company):
        ...
        Rule89Table5.iloc[:, :1] = Rule89Table5.iloc[:, :1].astype(float).applymap(lambda x: f"{x:,.0f}")
        Rule89Table5.iloc[:,1:] = Rule89Table5.iloc[:,1:].astype(float).applymap(lambda x: f"{x:.3f}")
    ...
    def buildRule89table7(self, company):
        ...
        Rule89Table7.iloc[:, 1:] = Rule89Table7.iloc[:,1:].applymap(lambda x: f"{x:.0f}")
    ...
    def buildRule89table8(self, company):
        ...
        Rule89Table8.iloc[:, :] = Rule89Table8.iloc[:, :].applymap(lambda x: f"{x:.3f}")
    ...
    def buildRule89tableB1b(self, company):
        ...
        output_table.iloc[:, :] = output_table.iloc[:, :].applymap(lambda x: f"{x:.2f}")
    ...
    def buildRule298table1(self, company):
        ...
        Rule98Table1.iloc[:,:1] = Rule98Table1.iloc[:,:1].astype(int).applymap(lambda x: "{:,}".format(x)) # Deductible Commas
        Rule98Table1.iloc[:, 1:] = Rule98Table1.iloc[:, 1:].astype(float).applymap(lambda x: f"{x:.3f}") # 3 Place float factors
    ...
    def buildRule298PPT(self,company):
        ...
        coll_deductible_map = pd.read_excel("BA input File.xlsx", sheet_name = "298 Coll Map").fillna("Y")
        otc_deductible_map = pd.read_excel("BA input File.xlsx", sheet_name = "298 OTC Map").fillna("Y")
        otc_fg_deductible_map = pd.read_excel("BA input File.xlsx", sheet_name="298 OTC FG Map").fillna("Y") # Full glass
        ...
        fg_state_list = pd.read_excel("BA input File.xlsx", sheet_name = "298 FG States")
    ...
    def buildRule298AutoBlanket(self,company):
        ...
        factor_table.iloc[:, 1:] = factor_table.iloc[:, 1:].astype(float).applymap(lambda x: f"{x:.3f}")
    ...
    def buildMotorcycles1(self, company):
        ...
        Motorcycles1.iloc[:, 1:] = Motorcycles1.iloc[:, 1:].astype(float).applymap(lambda x: f"{x:.3f}")
        return Motorcycles1
    ...
    def buildMotorcycles2(self, company):
        ...
        Motorcycles2.iloc[:, 1:] = Motorcycles2.iloc[:, 1:].astype(float).applymap(lambda x: f"{x:.3f}")
        return Motorcycles2
    ...
    def buildMotorcycles3(self, company):
        ...
        Motorcycles3.iloc[:, 1:] = Motorcycles3.iloc[:, 1:].astype(float).applymap(lambda x: f"{x:.3f}")
        return Motorcycles3
    ...
    def buildMotorcycles4(self, company):
        ...
        Motorcycles4.iloc[:, 1:] = Motorcycles4.iloc[:, 1:].astype(float).applymap(lambda x: f"{x:.3f}")
        return Motorcycles4
    ...
    def buildMotorcycles5(self, company):
        ...
        Motorcycles5.iloc[:, 1:] = Motorcycles5.iloc[:, 1:].astype(float).applymap(lambda x: f"{x:.3f}")
        return Motorcycles5
    ...
    def buildMedPayments92A(self, company):
        ...
        School_239 = pd.read_excel("BA input File.xlsx", sheet_name="239 School Buses")
        Other_239 = pd.read_excel("BA input File.xlsx", sheet_name="239 Other Buses")
        Van_239 = pd.read_excel("BA input File.xlsx", sheet_name="239 Van Pools")
        Taxi_239 = pd.read_excel("BA input File.xlsx", sheet_name="239 Taxis")
        TTT_222 = pd.read_excel("BA input File.xlsx", sheet_name="222 TTT")
        PPT_232 = pd.read_excel("BA input File.xlsx", sheet_name="232 PPT")
        ...
        result.iloc[:, 1:] = result.iloc[:, 1:].astype(float).applymap(lambda x: f"{x:.3f}")
    ...
    def buildMedPayments92B(self, company):
        ...
        MedPaymentsZone.iloc[:, 1:] = MedPaymentsZone.iloc[:, 1:].astype(float).applymap(lambda x: f"{x:.3f}")
    ...
    def buildCorporalPunish(self, company):
        ...
        CorporalPunish.iloc[:2,1:] = CorporalPunish.iloc[:2,1:].astype(float).applymap(lambda x: f"{x:.2f}")
        CorporalPunish.iloc[2:3,1:2] = CorporalPunish.iloc[2:3,1:2].astype(float).applymap(lambda x: f"{x:.0f}")
    ...
    def buildAntiqueAutoPDRates(self, company):
        ...
        coll_deductible_map = pd.read_excel("BA input File.xlsx", sheet_name = "266 Coll Map").fillna("Y")
        otc_deductible_map = pd.read_excel("BA input File.xlsx", sheet_name = "266 OTC Map").fillna("Y")
        factor_table = factor_table
    ...
    def buildLeasingOrRentalConcernsFactors(self, company):
        ...
        LeasingOrRentalConcernsFactors.iloc[:, 1:] = LeasingOrRentalConcernsFactors.iloc[:, 1:].astype(float).applymap(lambda x: f"{x:.0f}")
    ...
    def buildMobileHomeFactors(self, company):
        ...
        MobileHomeFactors.iloc[:, 1:] = MobileHomeFactors.iloc[:, 1:].astype(float).applymap(lambda x: f"{x:.3f}")
    ...
    def buildHiredAutoPDFactors(self, company):
        ...
        HiredAutoPDFactors.iloc[:-1, 1:] = HiredAutoPDFactors.iloc[:-1, 1:].applymap(safe_format)
    ...
    def buildBusinessInterruptionFactors(self, company):
        ...
        BusinessInterruptionFactors.iloc[:, 1:] = BusinessInterruptionFactors.iloc[:, 1:].astype(float).applymap(lambda x: f"{x:.2f}")
    ...
    def buildExtendedBusinessFactors(self, company):
        ...
        ExtendedBusinessFactors.iloc[:, 1:] = ExtendedBusinessFactors.iloc[:, 1:].astype(float).applymap(lambda x: f"{x:.3f}")
    ...
    def buildWaitingBusinessFactors(self, company):
        ...
        WaitingBusinessFactors.iloc[:, 1:] = WaitingBusinessFactors.iloc[:, 1:].astype(float).applymap(lambda x: f"{x:.3f}")
    ...
    def build101A1(self, company):
        ...
        Table101A1.iloc[:, 1:] = Table101A1.iloc[:, 1:].astype(float).applymap(lambda x: f"{x:.3f}")
        return Table101A1
    ...
    def build101A2(self, company):
        ...
        Table101A2.iloc[:, 1:] = Table101A2.iloc[:, 1:].astype(float).applymap(lambda x: f"{x:.3f}")
    ...
    def build101A3(self, company):
        ...
        Table101A3.iloc[:, 1:] = Table101A3.iloc[:, 1:].astype(float).applymap(lambda x: f"{x:.3f}")
    ...
    def build101A4(self, company):
        ...
        Table101A4.iloc[:, 1:] = Table101A4.iloc[:, 1:].astype(float).applymap(lambda x: f"{x:.3f}")
    ...
    def build101A5(self, company):
        ...
        Table101A5.iloc[:, 1:] = Table101A5.iloc[:, 1:].astype(float).applymap(lambda x: f"{x:.3f}")
    ...
    def buildILF(self, company):
        ...
        limits_list = pd.read_excel("BA Input File.xlsx", sheet_name="CSL Limits")
        ...
        ILF.iloc[:, 1:] = ILF.iloc[:, 1:].astype(float).applymap(lambda x: f"{x:.3f}")
    ...
    def buildDriveOtherFactors(self, company):
        ...
        um_info = pd.read_excel("BA Input File.xlsx", sheet_name = "297 Map")
        ...
        DriveOtherFactors.iloc[:,1:] = DriveOtherFactors.iloc[:,1:].astype(float).applymap(lambda x: f"{x:.3f}")
    ...
    def buildRentalFactors(self, company):
        ...
        RentalFactors.iloc[:,1:] = RentalFactors.iloc[:,1:].applymap(lambda x: f"{x:.2f}")
    ...
    def buildAudioFactors(self, company):
        ...
        AudioFactors = AudioFactors.rename(columns={'CostOfNew': 'Cost of Equipment', 'Premium': 'Rate'}) \
            .sort_values(by=['Rate'])
        AudioFactors.iloc[:, 1:] = AudioFactors.iloc[:, 1:].applymap(lambda x: f"{x:.0f}")
    ...
    def buildTapeFactors(self, company):
        ...
        TapeFactors.iloc[:, 1:] = TapeFactors.iloc[:, 1:].applymap(lambda x: f"{x:.2f}")
    ...
    def buildSnowMobileFactors(self, company):
        ...
        SnowMobileFactors.iloc[:-1,1:] = SnowMobileFactors.iloc[:-1,1:].astype(float).applymap(lambda x: f"{x:.2f}")
    ...
    def build297Table_unstacked(self, company, table_num: int):
        ...
        if self.StateAbb == "OH" and table_num == 10:
            state_mapper = pd.read_excel("BA Input File.xlsx", sheet_name=sheet_names[table_num])
            state_mapper = state_mapper[state_mapper["state"] == self.StateAbb]

        sheet_name = sheet_names[table_num]
        limits_list = pd.read_excel("BA Input File.xlsx", sheet_name="UM-UIM-UMPD Limits", header=[0, 1])
    ...
    def buildVAPCD(self,company):
        ...
        um_table.iloc[:,1:] = um_table.iloc[:,1:].astype(float).applymap(lambda x: f"{x:.3f}")
    ...
    def buildCT_A1(self, company):
        ...
        output_table = self.simple_long_table_build(self.rateTables, company, sheet_names, '', orig_values,
                                                    replace_values, filter_values=orig_values)
        output_table = output_table.rename(columns={output_table.columns[0]: '', output_table.columns[1]: ' '})
        output_table.iloc[:,1:] = output_table.iloc[:,1:].astype(float).applymap(lambda x: f"{x:.0f}")
    ...
    def buildMI_A2(self, company):
        ...
        output_table = self.simple_long_table_build(self.rateTables, company, sheet_names, '', orig_values,
                                                    replace_values, filter_values=orig_values)
        output_table = output_table.rename(columns={output_table.columns[0]: '', output_table.columns[1]: ''})
        output_table.iloc[:,1:] = output_table.iloc[:,1:].astype(float).applymap(lambda x: f"{x:.2f}")
    ...
    def buildMI_A4A(self, company):
        ...
        final_df.iloc[:, 1:] = final_df.iloc[:, 1:].astype(float).applymap(lambda x: f"{x:.2f}")
    ...
    def buildBAPages(self):
        ...
        state_sheet_exceptions = pd.read_excel("BA Input File.xlsx", sheet_name=None, engine='openpyxl')
        ...
        data.iloc[:,1:] = data.iloc[:,1:].astype(float).applymap(lambda x: f"{x:.3f}")
        ...
        file_path = 'BA Input File.xlsx'
        map_293 = pd.read_excel(file_path, sheet_name="293 Map", engine='openpyxl')
        ...
        file_path = 'BA Input File.xlsx'
        map_297 = pd.read_excel(file_path, sheet_name="297 Map", engine='openpyxl')
        ...
        leaf_list = pd.read_excel("BA Input File.xlsx", sheet_name="452 Leaf")
        ...
        # t1_exclude_list = pd.read_excel("BA Input File.xlsx", sheet_name = "T1 Exclude")
        ...
        if self.StateAbb == "CT":
            ...
            # CT: Rule R1
            df_raw = pd.read_excel('BA Input File.xlsx', sheet_name="CT R1", header=None)
            R1 = df_raw.fillna('')
