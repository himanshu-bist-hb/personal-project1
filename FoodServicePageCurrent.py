...
class Food:
    ...
    def buildFoodPage(self):
        ...
        FoodService = ExcelSettingsCurrent.Excel(state=self.state, programName='Food Service', nEffective=self.nEffective, rEffective=self.rEffective, companyList=companies)

        fontName = FoodService.getFontName()
        fontSize = FoodService.getFontSize()

        if 'NACO' in self.rateTables.keys():
            FoodService.generateWorksheet('BRNACO', 'FS Table 3.B.1. NW Assurance State Base Rates', self.buildBaseRates('NACO'), False, True)
        if 'NAFF' in self.rateTables.keys():
            FoodService.generateWorksheet('BRNAFF', 'FS Table 3.B.1. NW Affinity State Base Rates', self.buildBaseRates('NAFF'), False, True)
        ...
        if 'NICOF' in self.rateTables.keys():
            FoodService.generateWorksheet('BRNICOF', 'FS Table 3.B.1. NICOF State Base Rates', self.buildBaseRates('NICOF'), False, True)
        ...
        self.formatEndorsementCharge(FoodPages['PLUS'])
