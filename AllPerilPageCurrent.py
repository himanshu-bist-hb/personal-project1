# This module formats the All Peril State Page workbook in Excel
...
class AllPeril:
    ...
    def buildSprinklerFactor(self):
        ...
        return filteredSprinklerPeril.replace({'Class_Code_Min': self.classCodes}).rename(columns={'Class_Code_Min': 'Program', 'Bldg Sprinkler Factor': 'Building', 'BPP Sprinkler Factor': 'BPP'}).sort_values(by=['Program'])
    ...
    def buildWHDeductiblePerBuilding(self, coverage):
        ...
        if coverage.casefold() == 'building': # Case-insensitive comparison
            filteredWHDedBldg = whDedBldgPeril.query(f'{"`Peril TypeCode`".strip()} == "allperil" & Coverage == "Building" & WHDeductibleAmt != "1" & WHDeductibleAmt != "2" & WHDeductibleAmt != "5"'). \
            ...
            filteredWHDedBldg = whDedBldgPeril.query(f'{"`Peril TypeCode`".strip()} == "allperil" & Coverage == "BPP" & WHDeductibleAmt != "1" & WHDeductibleAmt != "2" & WHDeductibleAmt != "5"'). \
                    replace({'Class_Code_Min': self.classCodes}).rename(columns={'WHDeductibleAmt': 'Wind / Hail Deductible Amount', 'AmtOfInsurance': 'Amount of Insurance'})
    ...
    def buildWHDeductiblePercentage(self, coverage):
        ...
        if coverage.casefold() == 'building': # Case-insensitive comparison
            filteredWHDeductiblePercentage = whDedPeril.query(f'`Peril TypeCode` == "allperil" & Coverage == "Building" & (WHDeductibleAmt == "1" | WHDeductibleAmt == "2" | WHDeductibleAmt == "5")')
        elif coverage.casefold() == 'bpp': # Case-insensitive comparison
            filteredWHDeductiblePercentage = whDedPeril.query(f'`Peril TypeCode` == "allperil" & Coverage == "BPP" & (WHDeductibleAmt == "1" | WHDeductibleAmt == "2" | WHDeductibleAmt == "5")')
        return filteredWHDeductiblePercentage.replace({'Class_Code_Min': self.classCodes}).pivot(index='Wind / Hail Deductible', columns='Class_Code_Min', values='WH_Deductible_Building_Factor').reset_index('Wind / Hail Deductible')
    ...
    def buildFireAlarmFactor(self):
        ...
        return fireAlarmPeril.query(f'`Peril TypeCode` == "allperil" & FireAlarmType == "Central Station Fire Alarm"').replace({'Peril TypeCode': {'allperil': 'All Programs'}}).rename(columns={'Peril TypeCode': 'Program'}). \
            pivot(index='Program', columns='Coverage', values='FireAlarmFactor').rename(columns={'Business Income': 'Bus Inc'})
    ...
    def buildTenantsImprovements(self):
        ...
        return tenantsImprovementsPeril.query(f'`Peril TypeCode` == "allperil"').replace({'Peril TypeCode': {'allperil': 'All Programs'}}).rename(columns={'Peril TypeCode': 'Program', 'TIBFactor': 'Factor'}). \
    ...
    def formatMasonryVeneer(self, ws):
        ws.column_dimensions['A'].width = self.pixelsToInches(215)
    ...
    def buildAllPerilPage(self):
        ...
        AllPeril = ExcelSettingsCurrent.Excel(state=self.state, programName='All Peril', nEffective=self.nEffective, rEffective=self.rEffective, companyList=companies)

        fontName = AllPeril.getFontName()
        fontSize = AllPeril.getFontSize()

        AllPeril.generateWorksheet('SPR', 'AS, FS, H, O, R, S, W Table 3.C.2.a. Sprinkler Factor', self.buildSprinklerFactor(), False, True)
        AllPeril.generateWorksheet('PCBG', 'AS, FS, H, O, R, S, W Table 3.C.2.b. Protection Class Factor - Building', self.buildProtectionClass('Building'), False, True)
        AllPeril.generateWorksheet('PCPP', 'AS, FS, H, O, R, S, W Table 3.C.2.b. Protection Class Factor - BPP', self.buildProtectionClass('BPP'), False, True)
        AllPeril.generateWorksheet('MVBG', 'AS, FS, H, O, R, S, W Table 3.C.2.c.1. Masonry Veneer Factor - Building', self.buildMasonryVeneer('Building'), False, True)
        AllPeril.generateWorksheet('MVPP', 'AS, FS, H, O, R, S, W Table 3.C.2.c.1. Masonry Veneer Factor - BPP', self.buildMasonryVeneer('BPP'), False, True)
        AllPeril.generateWorksheet('PD', 'AS, FS, H, O, R, S, W Table 3.C.2.f. Property Deductible Factor', self.buildPropertyDeductible(), False, True)
        AllPeril.generateWorksheet('WHOBG', 'AS, FS, H, O, R, S, W Table 3.C.2.g.(1). Windstorm or Hail Deductible Factor - Per Occurrence Fixed Deductible Amount - Building', self.buildWHDeductibleFactor('Building'), False, True)
        AllPeril.generateWorksheet('WHOPP', 'AS, FS, H, O, R, S, W Table 3.C.2.g.(1). Windstorm or Hail Deductible Factor - Per Occurrence Fixed Deductible Amount - BPP', self.buildWHDeductibleFactor('BPP'), False, True)
        AllPeril.generateWorksheet('WHBBG', 'AS, FS, H, O, R, S, W Table 3.C.2.g.(2). Windstorm or Hail Deductible Factor - Per Building Fixed Deductible Amount - Building', self.buildWHDeductiblePerBuilding('Building'), False, True)
        AllPeril.generateWorksheet('WHBPP', 'AS, FS, H, O, R, S, W Table 3.C.2.g.(2). Windstorm or Hail Deductible Factor - Per Building Fixed Deductible Amount - BPP', self.buildWHDeductiblePerBuilding('BPP'), False, True)
        AllPeril.generateWorksheet('WHPBG', 'AS, FS, H, O, R, S, W Table 3.C.2.g.(3). Windstorm or Hail Deductible Factor - Percentage Deductible - Building', self.buildWHDeductiblePercentage('Building'), False, True)
        AllPeril.generateWorksheet('WHPPP', 'AS, FS, H, O, R, S, W Table 3.C.2.g.(3). Windstorm or Hail Deductible Factor - Percentage Deductible - BPP', self.buildWHDeductiblePercentage('BPP'), False, True)
        AllPeril.generateWorksheet('BA', 'AS, FS, H, O, R, S, W Table 3.C.2.h. Burglar Alarm Modifier', self.buildBurglarAlarmFactor(), False, True) 
        AllPeril.generateWorksheet('CSFA', 'AS, FS, H, O, R, S, W Table 3.C.2.i. Central Station Fire Alarm Modifier', self.buildFireAlarmFactor(), True, True)
        AllPeril.generateWorksheet('BABG', 'AS, FS, H, O, R, S, W Table 3.C.2.j. Building Age Modifier - Building', self.buildBuildingAgeModifier('Building'), False, True)
        AllPeril.generateWorksheet('BAPP', 'AS, FS, H, O, R, S, W Table 3.C.2.j. Building Age Modifier - BPP', self.buildBuildingAgeModifier('BPP'), False, True)
        AllPeril.generateWorksheet('BABI', 'FS Table 3.C.2.j. Building Age Modifier - Bus Inc', self.buildBuildingAgeModifier('Business Income'), False, True) # Business Income is only applicable in the food program
        AllPeril.generateWorksheet('AIBG', 'AS, FS, H, O, R, S, W Table 3.C.2.l.(1). AOI (Amount of Insurance) Relativity Factor - Building', self.buildBuildingAOI(), False, True)
        AllPeril.generateWorksheet('AIPP', 'AS, FS, H, O, R, S, W Table 3.C.2.l.(1). AOI (Amount of Insurance) Relativity Factor - BPP', self.buildBPPAOI(), False, True)
        AllPeril.generateWorksheet('BCEG', 'H Table 3.C.2.n., AS, FS, O, R, S, W Table 3.C.2.o. Building Code Effectiveness Grading', self.buildBCEG(), False, True)
        AllPeril.generateWorksheet('TIB', 'H Table 3.C.2.p., AS, FS, O, R, S, W Table 3.C.2.q. Tenants Improvements and Betterments Factor', self.buildTenantsImprovements(), False, True)

        AllPeril.createIndex()
        AllPerilPages = AllPeril.getWB()

        self.formatSprinklerFactor(AllPerilPages['SPR'])
        self.formatProtectionClass(AllPerilPages['PCBG'])
        self.formatProtectionClass(AllPerilPages['PCPP'])
        self.formatMasonryVeneer(AllPerilPages['MVBG'])
        self.formatMasonryVeneer(AllPerilPages['MVPP'])
        self.formatPropertyDeductible(AllPerilPages['PD'], Font(name=fontName, size=fontSize, bold=True))
        self.formatWHDeductibleFactor(AllPerilPages['WHOBG'], Font(name=fontName, size=fontSize, bold=True))
        self.formatWHDeductibleFactor(AllPerilPages['WHOPP'], Font(name=fontName, size=fontSize, bold=True))
        self.formatWHDeductiblePerBuilding(AllPerilPages['WHBBG'])
        self.formatWHDeductiblePerBuilding(AllPerilPages['WHBPP'])
        self.formatWHDeductiblePercentage(AllPerilPages['WHPBG'])
        self.formatWHDeductiblePercentage(AllPerilPages['WHPPP'])
        self.formatBurglarAlarm(AllPerilPages['BA'])
        self.formatFireAlarm(AllPerilPages['CSFA'])
        self.formatBuildingAge(AllPerilPages['BABG'])
        self.formatBuildingAge(AllPerilPages['BAPP'])
        self.formatBuildingAge(AllPerilPages['BABI'])
        self.formatAOI(AllPerilPages['AIBG'], Font(name=fontName, size=fontSize, bold=True))
        self.formatAOI(AllPerilPages['AIPP'], Font(name=fontName, size=fontSize, bold=True))
        self.formatBCEG(AllPerilPages['BCEG'], Font(name=fontName, size=fontSize, bold=True))
        self.formatTIB(AllPerilPages['TIB'])

        return AllPerilPages