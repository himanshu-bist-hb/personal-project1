# This module builds and formats the All Peril State Page workbook (Pre 2.0).
#
# The build*() methods below are unchanged business logic, transcribed from
# the root-level AllPerilPageCurrent.py. The pre-2.0 program differs from
# BP-2.0 in a few tables: there is no Territory Multiplier sheet, the
# percentage wind/hail deductible only covers the 1/2/5% deductibles, and
# the WH per-occurrence sheets use the older column profile (the
# WHOBG_CURRENT / WHOPP_CURRENT layout keys in BOP Input File.xlsx).
#
# All Excel formatting lives in ExcelSettingsBOP.py, driven by
# "BOP/BOP Input File.xlsx".
...
class AllPeril:
    ...
    def buildAllPerilPage(self, progress_callback=None):
        ...
        # See AllPerilPage.py — multi-building-code states reuse BCEG_MULTI.
        ...
        sheetSpecs = [
            ...
            ('WHPBG', 'AS, FS, H, O, R, S, W Table 3.C.2.g.(3). Windstorm or Hail Deductible Factor - Percentage Deductible - Building', lambda: self.buildWHDeductiblePercentage('Building'), False, True, 'WHPBG_AP_CURRENT'),
            ('WHPPP', 'AS, FS, H, O, R, S, W Table 3.C.2.g.(3). Windstorm or Hail Deductible Factor - Percentage Deductible - BPP', lambda: self.buildWHDeductiblePercentage('BPP'), False, True, 'WHPPP_AP_CURRENT'),
