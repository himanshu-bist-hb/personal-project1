# This module builds and formats the Office State Page workbook (pre-2.0).
#
# Same idea as OfficePage.py (BP-2.0) — see that file's module docstring —
# but transcribed from the root-level OfficePageCurrent.py, which predates
...
# Service/Retail's pre-2.0 pages) and does not have the cat4 exclusion BP-2.0
# added to buildBaseRates, nor the Veterinarian Specialized Endorsement (VS/
# VSPL/VPL), Mobile Pet and Veterinarian Services Endorsement (MPVS), Pet
# Services Specialized Endorsement (PSS/PSPL), or the Architects/Consultants/
# Professional Services/Accountants/Attorneys/Health Care Specialized
# Endorsements (AES/CS/PFSS/ACS/ATS/HCS) BP-2.0 added — this version still
# has the plain PLUS endorsement those replaced. Property Damage Liability
# Deductible and Liability Size of Risk are also unpivoted here (single
# "liability1" peril, not broken out by peril like the BP-2.0 version).
# Unlike Retail's pre-2.0 page, the root-level OfficePageCurrent.py already
# used the correct per-company argument in its own buildBaseRates() calls
# (no NGIC-hardcoded-for-every-company bug to fix here).
...
class Office:
    ...
    # fallback. See [[bop-nesting-order]] — the root-level OfficePageCurrent.py
    ...
    def buildOfficePage(self, progress_callback=None):
        ...
        sheetSpecs += [
            ('TRBG', 'O Table 3.C.1.a. State Territory Multiplier - Building', lambda: self.buildTerritoryMultiplier('Building'), False, True, 'PROGRAM_TR', None),
            ('TRPP', 'O Table 3.C.1.a. State Territory Multiplier - BPP', lambda: self.buildTerritoryMultiplier('BPP'), False, True, 'PROGRAM_TR', None),
            ('TRLB', 'O Table 3.C.1.a. State Territory Multiplier - Liability', lambda: self.buildTerritoryMultiplier('Liability'), False, True, 'PROGRAM_TR', None),
            ...
            ('LS', 'O Table 3.C.4.d. Liability Size of Risk Modifier', self.buildLiabilitySizeRisk, False, True, 'LS_RETAIL_CURRENT', None),
