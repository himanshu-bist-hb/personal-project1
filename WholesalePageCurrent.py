...
class Wholesale:
    ...
    def buildLiabilitySizeRisk(self):
        ...
        filteredLiabilitySizeRisk = liabilitySizeRisk.query(f'ClassCode_Min == {self.wholesaleProgramCode} & `Peril TypeCode` == "liability1"').rename(columns={'ReceiptMin': 'Min', 'ReceiptMax': 'Max', 'LiabilityFactor': 'Factor'})
        return filteredLiabilitySizeRisk.filter(items=['Min', 'Max', 'Factor']).fillna({'Max': 'and over'})
    ...
    def buildWholesalePage(self):
        ...
        if 'NAFF' in self.rateTables.keys():
            Wholesale.generateWorksheet('BRNAFF', 'W Table 3.B.1. NW Affinity State Base Rates', self.buildBaseRates('NAFF'), False, True)
        ...
        if 'NICOF' in self.rateTables.keys():
            Wholesale.generateWorksheet('BRNICOF', 'W Table 3.B.1. NICOF State Base Rates', self.buildBaseRates('NICOF'), False, True)
