...
class AllPrograms:
    ...
    def buildTerritoryDefinitions(self):
        ...
        #TerritoryDefs["Garage Keepers"] = 'A000A000A000A000'
    ...
    #    EQTerritoryDefsByST = EQTerritoryDefsByST.filter(items=['ZIP', 'COMM_EQ_CODE']).rename(columns={'ZIP' : 'Zip Code', 'COMM_EQ_CODE' : 'EQ Territory'})
    #    EQDedTier = self.buildDataFrame("BP7EarthquakeDeductibleTier")
    #    EQDedTier = EQDedTier.rename(columns={'EarthquakeTerritory' : 'EQ Territory', 'DeductibleTier' : 'Deductible Tier'})
    ...
    def buildPropertyDeductibleH(self):
        propertyDedPeril = self.buildDataFrame("BP7_Peril_PropertyDeductible")
        filteredPropertyDeductible = propertyDedPeril.query(f'Class_Code_Min == 10000 & `Peril TypeCode` in {self.perils}').replace({'Peril TypeCode': self.perilsConversions}). \
                rename(columns={'DeductibleAmount': 'Property Deductible', 'BPPTIB_AmtofInsurance_Min': 'BPP Min', 'BPPTIB_AmtOfInsurance_Max': 'BPP Max', 'BLDG_AmtOfInsurance': 'Building'})
        pivotedPropertyDeductible = filteredPropertyDeductible.filter(items=['Peril TypeCode', 'Property Deductible', 'BPP Min', 'BPP Max', 'Building', 'PropertyDeductibleFactor']). \
                pivot(index=['Property Deductible', 'BPP Min', 'BPP Max', 'Building'], columns='Peril TypeCode', values='PropertyDeductibleFactor').reset_index(['Property Deductible', 'BPP Min', 'BPP Max', 'Building'])
        sortedPropertyDeductible = pivotedPropertyDeductible.astype({'Property Deductible': 'int32'}).fillna({'BPP Max': '+'}).sort_values(by=['Property Deductible', 'BPP Min', 'BPP Max', 'Building'])
    ...
    def buildWHDeductiblePerBuilding(self, coverage):
        ...
        if coverage.casefold() == 'building': # Case-insensitive comparison
            filteredWHDedBldg = whDedBldgPeril.query(f'Class_Code_Min == 40000 & {"`Peril TypeCode`".strip()} in {self.perils} & Coverage == "Building" & WHDeductibleAmt != "1" & WHDeductibleAmt != "2" & WHDeductibleAmt != "3" & WHDeductibleAmt != "4" & WHDeductibleAmt != "5"'). \
            ...
            filteredWHDedBldg = whDedBldgPeril.query(f'Class_Code_Min == 40000 & {"`Peril TypeCode`".strip()} in {self.perils} & Coverage == "BPP" & WHDeductibleAmt != "1" & WHDeductibleAmt != "2" & WHDeductibleAmt != "3" & WHDeductibleAmt != "4" & WHDeductibleAmt != "5"'). \
                    replace({'Peril TypeCode': self.perilsConversions}).rename(columns={'WHDeductibleAmt': 'Wind / Hail Deductible Amount', 'AmtOfInsurance': 'Amount of Insurance'})
    ...
    def buildWHDeductiblePercentage(self, coverage):
        ...
        if coverage.casefold() == 'building': # Case-insensitive comparison
            filteredWHDeductiblePercentage = whDedPeril.query(f'Class_Code_Min == 40000 & `Peril TypeCode` in {self.perils} & Coverage == "Building" & AmtOfInsurance == 1 & (WHDeductibleAmt == "1" | WHDeductibleAmt == "2" | WHDeductibleAmt == "3" | WHDeductibleAmt == "4" | WHDeductibleAmt == "5")'). \
                    rename(columns={'AmtOfInsurance': 'Amount of Insurance'})
        elif coverage.casefold() == 'bpp': # Case-insensitive comparison
            filteredWHDeductiblePercentage = whDedPeril.query(f'Class_Code_Min == 40000 & `Peril TypeCode` in {self.perils} & Coverage == "BPP" & AmtOfInsurance == 1 & (WHDeductibleAmt == "1" | WHDeductibleAmt == "2" | WHDeductibleAmt == "3" | WHDeductibleAmt == "4" | WHDeductibleAmt == "5")'). \
                    rename(columns={'AmtOfInsurance': 'Amount of Insurance'})
    ...
    def buildWHDeductibleFactorH(self, coverage):
        whDedPeril = self.buildDataFrame("BP7_Peril_WH_Deductible_Factor")
        if coverage.casefold() == 'building': # Case-insensitive comparison
            filteredWHDeductibleFactor = whDedPeril.query(f'Class_Code_Min == 10000 & `Peril TypeCode` in {self.perils} & Coverage == "Building"'). \
                replace({'Peril TypeCode': self.perilsConversions}).rename(columns={'Peril TypeCode': 'Peril', 'BPPTIB_AmtofInsurance_Min': 'BPP Min', 'BPPTIB_AmtofInsurance_Max': 'BPP Max', 'BLDG_AmtofInsurance': 'Building'})
        elif coverage.casefold() == 'bpp': # Case-insensitive comparison
            filteredWHDeductibleFactor = whDedPeril.query(f'Class_Code_Min == 10000 & `Peril TypeCode` in {self.perils} & Coverage == "BPP"'). \
                replace({'Peril TypeCode': self.perilsConversions}).rename(columns={'Peril TypeCode': 'Peril', 'BPPTIB_AmtofInsurance_Min': 'BPP Min', 'BPPTIB_AmtofInsurance_Max': 'BPP Max', 'BLDG_AmtofInsurance': 'Building'})
        updatedWHDeductibleFactor = filteredWHDeductibleFactor[filteredWHDeductibleFactor['WH_PercentOrAmount'].notnull()] # Filtering out the missing values from the WH_PercentOrAmount column
        sortedWHDeductibleFactor = updatedWHDeductibleFactor.fillna({'BPP Max': '+'}).sort_values(by=['Peril', 'BPP Min', 'BPP Max', 'Building']) # Filling in missing values and sorting
        return sortedWHDeductibleFactor.astype({'WH_PercentOrAmount': 'int32'}).pivot(index=['Peril', 'BPP Min', 'BPP Max', 'Building'], columns='WH_PercentOrAmount', values='WH Factor').reset_index(['Peril', 'BPP Min', 'BPP Max', 'Building'])
    ...
    def buildWHDeductiblePerBuildingH(self, coverage):
        whDedBldgPeril = self.buildDataFrame("BP7 Peril_WH_Deductible_Per_Building")
        if coverage.casefold() == 'building': # Case-insensitive comparison
            filteredWHDedBldg = whDedBldgPeril.query(f'Class_Code_Min == 10000 & {"`Peril TypeCode`".strip()} in {self.perils} & Coverage == "Building" & WHDeductibleAmt != "1" & WHDeductibleAmt != "2" & WHDeductibleAmt != "3" & WHDeductibleAmt != "4" & WHDeductibleAmt != "5"'). \
                    replace({'Peril TypeCode': self.perilsConversions}).rename(columns={'WHDeductibleAmt': 'Wind / Hail Deductible Amount', 'AmtOfInsurance': 'Amount of Insurance'})
        elif coverage.casefold() == 'bpp': # Case-insensitive comparison
            filteredWHDedBldg = whDedBldgPeril.query(f'Class_Code_Min == 10000 & {"`Peril TypeCode`".strip()} in {self.perils} & Coverage == "BPP" & WHDeductibleAmt != "1" & WHDeductibleAmt != "2" & WHDeductibleAmt != "3" & WHDeductibleAmt != "4" & WHDeductibleAmt != "5"'). \
                    replace({'Peril TypeCode': self.perilsConversions}).rename(columns={'WHDeductibleAmt': 'Wind / Hail Deductible Amount', 'AmtOfInsurance': 'Amount of Insurance'})
        return filteredWHDedBldg.pivot(index=['Wind / Hail Deductible Amount', 'Amount of Insurance'], columns='Peril TypeCode', values='WH_Deductible_Building_Factor').reset_index(['Wind / Hail Deductible Amount', 'Amount of Insurance']). \
                astype({'Wind / Hail Deductible Amount': 'int32'}).sort_values(by=['Wind / Hail Deductible Amount', 'Amount of Insurance'])
    ...
    def buildWHDeductiblePercentageH(self, coverage):
        ...
        whDedPeril['Wind / Hail Deductible'] = whDedPeril['WHDeductibleAmt'] + '%'
        if coverage.casefold() == 'building': # Case-insensitive comparison
            filteredWHDeductiblePercentage = whDedPeril.query(f'Class_Code_Min == 10000 & `Peril TypeCode` in {self.perils} & Coverage == "Building" & AmtOfInsurance == 1 & (WHDeductibleAmt == "1" | WHDeductibleAmt == "2" | WHDeductibleAmt == "3" | WHDeductibleAmt == "4" | WHDeductibleAmt == "5")'). \
                    rename(columns={'AmtOfInsurance': 'Amount of Insurance'})
        elif coverage.casefold() == 'bpp': # Case-insensitive comparison
            filteredWHDeductiblePercentage = whDedPeril.query(f'Class_Code_Min == 10000 & `Peril TypeCode` in {self.perils} & Coverage == "BPP" & AmtOfInsurance == 1 & (WHDeductibleAmt == "1" | WHDeductibleAmt == "2" | WHDeductibleAmt == "3" | WHDeductibleAmt == "4" | WHDeductibleAmt == "5")'). \
                    rename(columns={'AmtOfInsurance': 'Amount of Insurance'})
        return filteredWHDeductiblePercentage.replace({'Peril TypeCode': self.perilsConversions}).pivot(index=['Wind / Hail Deductible'], columns='Peril TypeCode', values='WH_Deductible_Building_Factor').reset_index(['Wind / Hail Deductible'])
    ...
    def buildFireAlarmFactor(self):
        ...
        #fireAlarmPeril.loc[fireAlarmPeril['Peril'] == 'NC-Water', "Bus Inc"] = ""
        #fireAlarmPeril.loc[fireAlarmPeril['Peril'] == 'NW-Fire', "Bus Inc"] = ""
        #fireAlarmPeril.loc[fireAlarmPeril['Peril'] == 'NW-Other', "Bus Inc"] = ""
        #fireAlarmPeril.loc[fireAlarmPeril['Peril'] == 'NW-Theft', "Bus Inc"] = ""
        #fireAlarmPeril.loc[fireAlarmPeril['Peril'] == 'NW-Water', "Bus Inc"] = ""
    ...
    def buildBCEG(self):
        ...
        #return filteredBCEG.pivot(index='Tier Grade', columns='Peril TypeCode', values='BCEGFactor').reset_index('Tier Grade').rename(columns={'Tier Grade': 'Grade'}). \
                #replace({'Grade': {'1': '01', '2': '02', '3': '03', '4': '04', '5': '05', '6': '06', '7': '07', '8': '08', '9': '09', '98': 'Non-Particip', '99': 'Ungraded'}}).sort_values(by=['Grade']).drop('L-Products', axis=1)
    ...
    def buildAllProgramsPage(self):
        ...
        AllPrograms = ExcelSettings.Excel(state=self.state, programName='All Programs', nEffective=self.nEffective, rEffective=self.rEffective, companyList=companies)
        ...
        self.formatProtectionClass(AllProgramsPages['PCBI'])
        ...
        self.formatPropertyDeductible(AllProgramsPages['PDH'], Font(name=fontName, size=fontSize, bold=True))
        self.formatWHDeductibleFactor(AllProgramsPages['WHOBG'], Font(name=fontName, size=fontSize, bold=True))
        self.formatWHDeductibleFactor(AllProgramsPages['WHOBGH'], Font(name=fontName, size=fontSize, bold=True))
        self.formatWHDeductibleFactor(AllProgramsPages['WHOPP'], Font(name=fontName, size=fontSize, bold=True))
        self.formatWHDeductibleFactor(AllProgramsPages['WHOPPH'], Font(name=fontName, size=fontSize, bold=True))
        self.formatWHDeductiblePerBuilding(AllProgramsPages['WHBBG'])
        self.formatWHDeductiblePerBuilding(AllProgramsPages['WHBBGH'])
        self.formatWHDeductiblePerBuilding(AllProgramsPages['WHBPP'])
        self.formatWHDeductiblePerBuilding(AllProgramsPages['WHBPPH'])
        self.formatWHDeductiblePercentage(AllProgramsPages['WHPBG'])
        self.formatWHDeductiblePercentage(AllProgramsPages['WHPBGH'])
        self.formatWHDeductiblePercentage(AllProgramsPages['WHPPP'])
        self.formatWHDeductiblePercentage(AllProgramsPages['WHPPPH'])
        ...
        self.formatTerritoryMultiplier(AllProgramsPages['TR'])
