...
class Retail:
    ...
    def buildRetailPage(self):
        ...
        Retail = ExcelSettingsCurrent.Excel(state=self.state, programName='Retail', nEffective=self.nEffective, rEffective=self.rEffective, companyList=companies)
        ...
        if 'NACO' in self.rateTables.keys():
            Retail.generateWorksheet('BRNACO', 'R Table 3.B.1. NW Assurance State Base Rates', self.buildBaseRates('NACO'), False, True)
        if 'NAFF' in self.rateTables.keys():
            Retail.generateWorksheet('BRNAFF', 'R Table 3.B.1. NW Affinity State Base Rates', self.buildBaseRates('NAFF'), False, True)
        ...
        if 'NICOF' in self.rateTables.keys():
            Retail.generateWorksheet('BRNICOF', 'R Table 3.B.1. NICOF State Base Rates', self.buildBaseRates('NICOF'), False, True)
        ...
        self.formatEndorsementCharge(RetailPages['PLUS'])
