# This module builds and formats the Retail State Page workbook (pre-2.0).
#
# Same idea as RetailPage.py (BP-2.0) — see that file's module docstring —
# but transcribed from the root-level RetailPageCurrent.py, which predates
...
# Service's pre-2.0 pages) and does not have the cat4/L-Products exclusions
# BP-2.0 added, nor the Pet Services (PSS/PSPL) or Retail Trade Specialized
# (RTS) endorsements — this version still has the plain PLUS endorsement
# those replaced. Property Damage Liability Deductible and Liability Size
# of Risk are also unpivoted here (single "liability1" peril, not broken out
# by peril like the BP-2.0 version).
...
class Retail:
    ...
    # fallback. See [[bop-nesting-order]] — the root-level RetailPageCurrent.py
    ...
    def buildBaseRates(self, company):
        ...
        filteredBuilingBaseRates = buildingBaseRates.query(f'Class_Code_Min == {self.retailProgramCode} & `Peril TypeCode` in {self.perils}').filter(items=['Peril TypeCode', 'BuildingBaseRate'])
        filteredBPPBaseRates = bppBaseRates.query(f'Class_Code_Min == {self.retailProgramCode} & `Peril TypeCode` in {self.perils}').filter(items=['Peril TypeCode', 'BPPBaseRate'])
    ...
    def buildConstructionType(self, coverage):
        ...
        if coverage.casefold() == 'building':
            return filteredConstructionType.pivot(index='Construction', columns='Peril TypeCode', values='BldgConstructionClassFactor').reset_index('Construction').drop(columns=['WF', 'NC-BINC'], errors='ignore')
        elif coverage.casefold() == 'bpp':
            return filteredConstructionType.pivot(index='Construction', columns='Peril TypeCode', values='BPPConstructionClassFactor').reset_index('Construction').drop(columns=['WF', 'NC-BINC'], errors='ignore')
    ...
    def buildYearBuiltModifier(self, coverage):
        ...
        if coverage.casefold() == 'building':
            return filteredYearBuiltModifier.pivot(index='Year Built Range', columns='Peril TypeCode', values='Bldg_Year_Built_Factor').reset_index('Year Built Range').drop(columns=['WF', 'NC-BINC'], errors='ignore')
        elif coverage.casefold() == 'bpp':
            return filteredYearBuiltModifier.pivot(index='Year Built Range', columns='Peril TypeCode', values='BPP_Year_Built_Factor').reset_index('Year Built Range').drop(columns=['WF', 'NC-BINC'], errors='ignore')
    ...
    def buildPDDeductibleAmount(self):
        ...
        return pdDeductibleAmount.query(f'ClassCode_Min == {self.retailProgramCode} & `Peril TypeCode` == "liability1"').rename(columns={'PDDeductibleAmount': 'P.D. Deductible Amount', 'PDDeductibleFactor': 'Factor'}). \
    ...
    def buildLiabilitySizeRisk(self):
        ...
        filteredLiabilitySizeRisk = liabilitySizeRisk.query(f'ClassCodeMin == {self.retailProgramCode} & `Peril TypeCode` == "liability1"').rename(columns={'BldgBPPLimit_Min': 'Limit Min', 'BldgBPPLimit_Max': 'Limit Max', 'LiabilityFactor': 'Factor'})
    ...
    def buildRetailPage(self, progress_callback=None):
        ...
        sheetSpecs += [
            ('TRBG', 'R Table 3.C.1.a. State Territory Multiplier - Building', lambda: self.buildTerritoryMultiplier('Building'), False, True, 'PROGRAM_TR', None),
            ('TRPP', 'R Table 3.C.1.a. State Territory Multiplier - BPP', lambda: self.buildTerritoryMultiplier('BPP'), False, True, 'PROGRAM_TR', None),
            ('TRLB', 'R Table 3.C.1.a. State Territory Multiplier - Liability', lambda: self.buildTerritoryMultiplier('Liability'), False, True, 'PROGRAM_TR', None),
