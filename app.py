    ...
    # ══════════════════════════════════════════════════════════════════════════
        ...
        with R:
            ...
            if st.session_state.multi_step == "idle":
                ...
                from BA.BARatePages import run as run_rate_pages, generate_pdf_only
...
# ─── FARM AUTO ───────────────────────────────────────────────────────────────
    ...
    tc1, tc2, _ = st.columns([2, 2, 8])
    with tc1:
        if st.button("Individual State", key="fa_btn_ind", use_container_width=True,
                     type="primary" if mode == "individual" else "secondary"):
            if mode != "individual":
                st.session_state.fa_mode = "individual"
                st.session_state.fa_confirm_step = "idle"
                st.rerun()
    with tc2:
        if st.button("Multiple States", key="fa_btn_mul", use_container_width=True,
                     type="primary" if mode == "multiple" else "secondary"):
            if mode != "multiple":
                st.session_state.fa_mode = "multiple"
                st.session_state.fa_multi_step = "idle"
                st.rerun()
        ...
        with L:
            ...
            if uploaded:
                ...
                for key in ALL_KEYS:
                    files = grouped.get(key, [])
                    if len(files) == 1:
                        st.session_state[f"fa_file_{key}"] = {"name": files[0].name, "bytes": files[0].read()}
                    elif len(files) > 1:
            ...
            if any(st.session_state[f"fa_file_{k}"] for k in ALL_KEYS):
                spacer(8)
                _, clr = st.columns([5, 1])
                with clr:
                    if st.button("Clear all", type="secondary"):
                        for k in ALL_KEYS: st.session_state[f"fa_file_{k}"] = None
                        st.session_state.fa_upload_reset += 1
                        st.session_state.fa_run_status = "idle"
                        st.rerun()

        with R:
            st.markdown('<div class="sec-label">&#9881; &nbsp;Configuration</div>', unsafe_allow_html=True)
            st.markdown('<p class="f-label">&#128193; &nbsp;Save Location</p>', unsafe_allow_html=True)
            typed = st.text_input("save_path", value=st.session_state.fa_save_dir, placeholder="Paste path or click Browse", label_visibility="collapsed")
            if typed != st.session_state.fa_save_dir: st.session_state.fa_save_dir = typed
            if st.button("Browse", key="fa_browse_btn"):
                folder = browse_folder()
                if folder: st.session_state.fa_save_dir = folder; st.rerun()
            if st.session_state.fa_save_dir:
                p = st.session_state.fa_save_dir
                st.markdown(f'<p class="f-ok">&#10003; &nbsp;{("…"+p[-38:]) if len(p)>40 else p}</p>', unsafe_allow_html=True)
            else:
                st.markdown('<p class="f-hint">Browse your device or paste the full folder path</p>', unsafe_allow_html=True)
            ...
            nc, pc = st.columns([3, 1])
            with nc:
                tm = st.number_input("mod_num", min_value=0, max_value=100, value=st.session_state.fa_sched_mod, step=1, label_visibility="collapsed")
                if tm != st.session_state.fa_sched_mod: st.session_state.fa_sched_mod = int(tm)
            with pc:
                st.markdown(f'<div style="display:flex;align-items:center;height:42px;padding-left:4px;"><span style="font-size:22px;font-weight:700;color:#1A5DAB;line-height:1;">{st.session_state.fa_sched_mod}<span style="font-size:13px;font-weight:400;color:#6B7A9E;">%</span></span></div>', unsafe_allow_html=True)
            spacer(6)
            sv = st.slider("mod_slider", 0, 100, value=st.session_state.fa_sched_mod, step=1, format="%d%%", label_visibility="collapsed")
            if sv != st.session_state.fa_sched_mod: st.session_state.fa_sched_mod = sv; st.rerun()
            st.markdown('<p class="f-hint">Rule 417 &middot; State Schedule Rating Maximum Modification Threshold</p>', unsafe_allow_html=True)

            spacer(6)
            st.markdown('<div class="sec-label">&#128203; &nbsp;Readiness</div>', unsafe_allow_html=True)
            has_files = any_req(); save_ok = bool(st.session_state.fa_save_dir)
            nr_now = n_req(); sdv = st.session_state.fa_save_dir; mv = st.session_state.fa_sched_mod
            req_sub  = f"All {len(REQUIRED)} ratebooks uploaded" if all_req() else f"{nr_now} of {len(REQUIRED)} ratebooks uploaded"
            save_sub = (("…"+sdv[-36:]) if len(sdv)>38 else sdv) if save_ok else "Not yet selected"
            ...
            if st.session_state.fa_confirm_step == "idle":
                ...
                bc1, bc2 = st.columns(2)
                with bc1:
                    if st.button("Cancel", key="fa_cancel_btn", use_container_width=True, type="secondary"):
                        st.session_state.fa_confirm_step = "idle"; st.rerun()
                with bc2:
                    if st.button("Proceed", key="fa_proceed_btn", use_container_width=True, type="primary"):
                        st.session_state.fa_confirm_step = "processing"; st.rerun()

            elif st.session_state.fa_confirm_step == "processing":
                st.markdown('<div class="btn-wait">', unsafe_allow_html=True)
                st.button("Processing Excel...", key="fa_run_btn_proc", use_container_width=True, disabled=True)
                st.markdown('</div>', unsafe_allow_html=True)
                loader_ph = st.empty()
                def update_progress(msg):
                    loader_ph.markdown(f'<div class="inline-loader"><div class="spin-ring"></div><div><div class="loader-label">Creating Excel rate pages…</div><div class="loader-sub">{msg}</div></div></div>', unsafe_allow_html=True)
                update_progress("Please wait while the workbooks are processed.")
                from FA.FARatePages import run as run_rate_pages
                try:
                    def _rb(k):
                    ...
                    xlsx_out, pdf_out = run_rate_pages(
                        ...
                        progress_callback=update_progress, skip_pdf=True)
                    st.session_state.fa_xlsx_path = xlsx_out; st.session_state.fa_pdf_path = pdf_out
                    st.session_state.fa_run_status = "success"; st.session_state.fa_pdf_status = "idle"
                except Exception as e:
                    import traceback; traceback.print_exc()
                    st.session_state.fa_run_status = "error"; st.session_state.fa_run_msg = str(e)
                st.session_state.fa_confirm_step = "idle"; st.rerun()

            elif st.session_state.fa_confirm_step == "pdf_processing":
                st.markdown('<div class="btn-wait">', unsafe_allow_html=True)
                st.button("Generating PDF...", key="fa_pdf_btn_proc", use_container_width=True, disabled=True)
                st.markdown('</div>', unsafe_allow_html=True)
                loader_ph2 = st.empty()
                def update_pdf_progress(msg):
                    loader_ph2.markdown(f'<div class="inline-loader"><div class="spin-ring"></div><div><div class="loader-label">Converting to PDF…</div><div class="loader-sub">{msg}</div></div></div>', unsafe_allow_html=True)
                from FA.FARatePages import generate_pdf_only
                try:
                    generate_pdf_only(st.session_state.fa_xlsx_path, st.session_state.fa_pdf_path, progress_callback=update_pdf_progress)
                    st.session_state.fa_pdf_status = "success"
                except Exception as e:
                    import traceback; traceback.print_exc()
                    st.session_state.fa_pdf_status = "error"; st.session_state.fa_run_msg = str(e)
                st.session_state.fa_confirm_step = "idle"; st.rerun()

            if st.session_state.fa_run_status == "success":
                spacer(10)
                st.success(f"&#10003;  Excel created: {Path(st.session_state.fa_xlsx_path).name}")
                if st.session_state.fa_pdf_status != "success":
                    st.markdown('<div class="btn-ready">', unsafe_allow_html=True)
                    if st.button("Generate PDF Document", key="fa_gen_pdf_btn", use_container_width=True):
                        st.session_state.fa_confirm_step = "pdf_processing"; st.rerun()
                    st.markdown('</div>', unsafe_allow_html=True)
                    if st.session_state.fa_pdf_status == "error":
                        st.error(f"PDF Error: {st.session_state.fa_run_msg}")
                else:
                    st.success(f"&#10003;  PDF created: {Path(st.session_state.fa_pdf_path).name}")
            elif st.session_state.fa_run_status == "error":
                spacer(10); st.error(st.session_state.fa_run_msg)
    ...
    # ══════════════════════════════════════════════════════════════════════════
        ...
        with L:
            ...
            st.markdown('<p class="f-hint">Select the folder that contains one subfolder per state. Each subfolder must contain that state\'s ratebook files.</p>', unsafe_allow_html=True)
            spacer(4)
            typed_src = st.text_input("multi_src_path", value=st.session_state.fa_multi_src_dir, placeholder="Paste path or click Browse", label_visibility="collapsed", key="fa_multi_src_input")
            if typed_src != st.session_state.fa_multi_src_dir:
                st.session_state.fa_multi_src_dir = typed_src; st.session_state.fa_multi_step = "idle"; st.session_state.fa_multi_results = []; st.session_state.fa_multi_sched_map = {}
            if st.button("Browse", key="fa_browse_src_btn"):
                folder = browse_folder()
                if folder: st.session_state.fa_multi_src_dir = folder; st.session_state.fa_multi_step = "idle"; st.session_state.fa_multi_results = []; st.session_state.fa_multi_sched_map = {}; st.rerun()

            if st.session_state.fa_multi_src_dir:
                src_path = Path(st.session_state.fa_multi_src_dir)
                if src_path.exists() and src_path.is_dir():
                    p = str(src_path)
                    st.markdown(f'<p class="f-ok">&#10003; &nbsp;{("…"+p[-38:]) if len(p)>40 else p}</p>', unsafe_allow_html=True)
                    spacer(14)
        ...
        with R:
            ...
            st.markdown('<p class="f-hint">Each state\'s Excel will be saved in <code>Save Location / State /</code></p>', unsafe_allow_html=True)
            spacer(4)
            typed_save = st.text_input("multi_save_path", value=st.session_state.fa_multi_save_dir, placeholder="Paste path or click Browse", label_visibility="collapsed", key="fa_multi_save_input")
            if typed_save != st.session_state.fa_multi_save_dir: st.session_state.fa_multi_save_dir = typed_save
            if st.button("Browse", key="fa_browse_save_btn"):
                folder = browse_folder()
                if folder: st.session_state.fa_multi_save_dir = folder; st.rerun()
            if st.session_state.fa_multi_save_dir:
                p = st.session_state.fa_multi_save_dir
                st.markdown(f'<p class="f-ok">&#10003; &nbsp;{("…"+p[-38:]) if len(p)>40 else p}</p>', unsafe_allow_html=True)
            else:
                st.markdown('<p class="f-hint">Choose where all output files will be saved.</p>', unsafe_allow_html=True)
            ...
            # ── Mode toggle ─────────────────────────────────────────────────
            _sched_opts = ["Upload Excel", "Enter Manually"]
            _sched_idx  = 0 if st.session_state.fa_multi_sched_mode == "upload" else 1
            _sched_pick = st.radio(
                "sched_mode_pick",
                options=_sched_opts,
                index=_sched_idx,
                horizontal=True,
                label_visibility="collapsed",
                key="fa_multi_sched_mode_radio",
            )
            _new_mode = "upload" if _sched_pick == "Upload Excel" else "manual"
            if _new_mode != st.session_state.fa_multi_sched_mode:
                st.session_state.fa_multi_sched_mode = _new_mode
                st.rerun()

            # ── Detect states early for this section ────────────────────────
            _src_ok_sched = bool(st.session_state.fa_multi_src_dir) and Path(st.session_state.fa_multi_src_dir).is_dir()
            _states_sched = scan_states(st.session_state.fa_multi_src_dir) if _src_ok_sched else []

            # ── Upload Excel branch ─────────────────────────────────────────
            if st.session_state.fa_multi_sched_mode == "upload":
                spacer(4)
                st.markdown(
                    '<p class="f-hint"><b>Col A</b> = State name &nbsp;&middot;&nbsp; '
                    '<b>Col B</b> = Schedule Rating % &nbsp;(no header row required).<br>'
                ...
                _sched_file = st.file_uploader(
                    "Upload schedule rating Excel",
                    type=["xlsx", "xls"],
                    key="fa_multi_sched_excel_uploader",
                    label_visibility="collapsed",
                )
                if _sched_file is not None:
                    import pandas as _pd
                    try:
                        ...
                        for _, _row in _df.iterrows():
                            try:
                                ...
                                pass
                        if _new_excel != st.session_state.fa_multi_sched_excel:
                            st.session_state.fa_multi_sched_excel = _new_excel
                            # Pre-populate sched_map for matched detected states
                            for _s in _states_sched:
                                _nm = _s["name"]
                                if _nm in _new_excel:
                                    st.session_state.fa_multi_sched_map[_nm] = _new_excel[_nm]
                            st.rerun()
                    except Exception as _exc:
                        st.markdown(f'<p style="font-size:11px;color:#C8102E;margin:4px 0;">&#9888; Could not parse file: {_exc}</p>', unsafe_allow_html=True)
            ...
            if _states_sched:
                ...
                _n_set = sum(
                    1 for _s in _states_sched
                    if st.session_state.fa_multi_sched_map.get(_s["name"], 0) > 0
                )
                # Apply any excel defaults not yet in sched_map
                if st.session_state.fa_multi_sched_excel and st.session_state.fa_multi_sched_mode == "upload":
                    for _s in _states_sched:
                        _nm = _s["name"]
                        if _nm not in st.session_state.fa_multi_sched_map and _nm in st.session_state.fa_multi_sched_excel:
                            st.session_state.fa_multi_sched_map[_nm] = st.session_state.fa_multi_sched_excel[_nm]
                ...
                for _i, _s in enumerate(_states_sched):
                    _nm  = _s["name"]
                    _exc = st.session_state.fa_multi_sched_excel.get(_nm)
                    _cur = int(st.session_state.fa_multi_sched_map.get(_nm, _exc if _exc is not None else 0))

                    _bg   = "#F9FBF9" if _i % 2 == 0 else "#FFFFFF"
                    _src_badge = ""
                    if _exc is not None and st.session_state.fa_multi_sched_mode == "upload":
                        _src_badge = ' &nbsp;<span style="font-size:8px;font-weight:700;color:var(--ok-fg);letter-spacing:0.5px;text-transform:uppercase;">excel</span>'
                    ...
                    with _ic:
                        ...
                        if int(_nv) != st.session_state.fa_multi_sched_map.get(_nm):
                            st.session_state.fa_multi_sched_map[_nm] = int(_nv)
                    with _pc:
                        st.markdown(
                            f'<div style="display:flex;align-items:center;height:38px;font-size:12px;'
                            f'color:var(--muted);">%</div>',
            ...
            spacer(6)
            gen_pdf = st.checkbox("Generate PDF for each state", value=st.session_state.fa_multi_gen_pdf, key="fa_multi_pdf_chk")
            if gen_pdf != st.session_state.fa_multi_gen_pdf: st.session_state.fa_multi_gen_pdf = gen_pdf
            if gen_pdf:
                st.markdown('<p class="f-hint">PDFs saved to <code>Save Location / PDF /</code></p>', unsafe_allow_html=True)

            spacer(6)
            st.markdown('<div class="sec-label">&#128203; &nbsp;Readiness</div>', unsafe_allow_html=True)
            src_ok      = bool(st.session_state.fa_multi_src_dir) and Path(st.session_state.fa_multi_src_dir).is_dir()
            save_ok_m   = bool(st.session_state.fa_multi_save_dir)
            states_list = scan_states(st.session_state.fa_multi_src_dir) if src_ok else []
            n_ready_m   = sum(1 for s in states_list if s["ready"])
            ready_m     = src_ok and save_ok_m and n_ready_m > 0
            ...
            if st.session_state.fa_multi_step == "idle":
                if ready_m:
                    ...
                    missing_m = (["source folder"] if not src_ok else (["no ready states"] if not n_ready_m else [])) + (["save location"] if not save_ok_m else [])
                    st.markdown('<div class="btn-wait">', unsafe_allow_html=True)
                    st.button(f"Waiting \u2014 {', '.join(missing_m)}", key="fa_multi_run_dis", use_container_width=True, disabled=True)
                    st.markdown('</div>', unsafe_allow_html=True)
                ...
                mc1, mc2 = st.columns(2)
                with mc1:
                    if st.button("Cancel", key="fa_multi_cancel_btn", use_container_width=True, type="secondary"):
                        st.session_state.fa_multi_step = "idle"; st.rerun()
                with mc2:
                    if st.button("Proceed", key="fa_multi_proceed_btn", use_container_width=True, type="primary"):
                        st.session_state.fa_multi_step = "processing"; st.rerun()

            elif st.session_state.fa_multi_step == "processing":
                st.markdown('<div class="btn-wait">', unsafe_allow_html=True)
                st.button("Processing states...", key="fa_multi_proc_btn", use_container_width=True, disabled=True)
                st.markdown('</div>', unsafe_allow_html=True)

                from FA.FARatePages import run as run_rate_pages, generate_pdf_only
                src_path_m   = Path(st.session_state.fa_multi_src_dir)
                save_path_m  = Path(st.session_state.fa_multi_save_dir)
                states_to_run = [s for s in scan_states(src_path_m) if s["ready"]]
                pdf_dir = save_path_m / "PDF"
                if st.session_state.fa_multi_gen_pdf: pdf_dir.mkdir(parents=True, exist_ok=True)
                ...
                state_ph.empty()
                st.session_state.fa_multi_results = results
                st.session_state.fa_multi_step = "done"
                st.rerun()

            elif st.session_state.fa_multi_step == "done":
                results   = st.session_state.fa_multi_results
                n_success = sum(1 for r in results if not r["error"])
                n_failed  = len(results) - n_success
                if n_failed == 0:
                    st.success(f"&#10003;  All {n_success} state{'s' if n_success!=1 else ''} completed successfully")
                ...
                st.markdown(f'<div class="assign-wrap"><div class="assign-hdr"><span>Results</span><span>{n_success}/{len(results)} completed</span></div>{result_rows}</div>', unsafe_allow_html=True)
                spacer(8)
                if st.button("Start Over", key="fa_multi_reset_btn", type="secondary"):
                    st.session_state.fa_multi_step = "idle"; st.session_state.fa_multi_results = []; st.rerun()
...
def _cmp_build_sidebyside_excel(old_sheets, new_sheets, new_only, deleted_only, sheet_stats, sheet_data, old_fn, new_fn, old_raw, new_raw) -> bytes:
    ...
    def _apply_sbs_dims(ws, new_src_ws, old_src_ws, nc_new, sep_col, nc_old):
        """Copy row heights and column widths from source workbooks.
        LEFT columns (1..nc_new)            → from proposed (new_src_ws)
        RIGHT columns (sep_col+1..+nc_old)  → from current  (old_src_ws), offset-mapped
        Separator column                    → fixed width 3
        """
        old_start = sep_col + 1
        # Row heights — use proposed as the primary source
        _row_src = new_src_ws or old_src_ws
        if _row_src:
            try:
                for row_idx, rd in _row_src.row_dimensions.items():
                    if rd.height is not None:
                        ws.row_dimensions[row_idx].height = rd.height
            except Exception: pass
        # LEFT side column widths from proposed
        if new_src_ws:
            try:
                for col_ltr, cd in new_src_ws.column_dimensions.items():
                    if cd.width is not None:
                        ws.column_dimensions[col_ltr].width = cd.width
            except Exception: pass
        # RIGHT side column widths from current (source col j → dest col old_start+j-1)
        if old_src_ws:
            try:
                for col_ltr, cd in old_src_ws.column_dimensions.items():
                    if cd.width is not None:
                        col_idx = openpyxl.utils.column_index_from_string(col_ltr)
                        dst_ltr = get_column_letter(old_start + col_idx - 1)
                        ws.column_dimensions[dst_ltr].width = cd.width
            except Exception: pass
        # Separator always narrow
        ws.column_dimensions[get_column_letter(sep_col)].width = 3

    def _finalise(ws, sep_col, src_wb, src_name):
        # No auto_width — dimensions are set by _apply_sbs_dims before this call
    ...
    for name in all_names:
        ...
        if name in new_only:
            ...
            for i, row_vals in enumerate(dataframe_to_rows(df, index=False, header=False), start=1):
                for jj, val in enumerate(row_vals, start=1):
                    c = ws.cell(i, jj, None if val=="" else val)
                    _cmp_copy_cell_style(src_ws.cell(i, jj) if src_ws else None, c)
                    c.fill = _CMP_FILL_SBS_NEW_SHT
                ws.cell(i, sep_col).fill = _CMP_FILL_SBS_SEP
                for j in range(old_start, old_start+nc_new): ws.cell(i, j)
            _apply_sbs_dims(ws, src_ws, None, nc_new, sep_col, nc_new)
            ...
            for i, row_vals in enumerate(dataframe_to_rows(df, index=False, header=False), start=1):
                ...
                ws.cell(i, sep_col).fill = _CMP_FILL_SBS_SEP
                for jj, val in enumerate(row_vals, start=old_start):
                    c = ws.cell(i, jj, None if val=="" else val)
                    _cmp_copy_cell_style(src_ws.cell(i, jj - old_start + 1) if src_ws else None, c)
                    c.fill = _CMP_FILL_SBS_DEL_SHT
            _apply_sbs_dims(ws, None, src_ws, nc_old, sep_col, nc_old)
            ...
            for i in range(nr):
                rs = row_status.get(i,"same")
                excel_row = i + 1
                # LEFT: NEW (proposed) — copy proposed formatting, overlay change highlights
                for j in range(nc_new):
                    ...
                    if rs=="added":     c.fill = _CMP_FILL_ADDED
                    elif rs=="deleted": c.value = None
                    elif cs=="changed": c.fill = _CMP_FILL_SBS_CHG_NEW
                ws.cell(excel_row, sep_col).fill = _CMP_FILL_SBS_SEP
                # RIGHT: OLD (current) — copy current formatting, overlay change highlights
                for j in range(nc_old):
                    val  = _cmp_cell_str(old_a.iat[i,j]) if i < len(old_a) else ""
                    cs   = cell_status.get((i,j),"same")
                    c    = ws.cell(excel_row, old_start+j, None if val=="" else val)
                    src_c = old_src_ws.cell(excel_row, j+1) if old_src_ws else None
                    _cmp_copy_cell_style(src_c, c)
                    if rs=="deleted":   c.fill = _CMP_FILL_DELETED
                    elif rs=="added":   c.value = None
                    elif cs=="changed": c.font = _cmp_font_with_strike(src_c.font if src_c else None)
            _apply_sbs_dims(ws, new_src_ws, old_src_ws, nc_new, sep_col, nc_old)
            _finalise(ws, sep_col, new_src_wb, name)
    buf = io.BytesIO(); wb.save(buf); return buf.getvalue()
