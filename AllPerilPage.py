...
class AllPeril:
    ...
    def buildBCEG(self):
        ...
        filteredBCEG = bceg.query(f'`Peril TypeCode` == "allperil"').copy()  #.rename(columns={'Tier Grade': 'Grade', 'BCEGFactor': 'All Programs'}).filter(items=['Grade', 'All Programs']) # Creating a new copy of the data here
        #return filteredBCEG.replace({'Grade': {'1': '01', '2': '02', '3': '03', '4': '04', '5': '05', '6': '06', '7': '07', '8': '08', '9': '09', '98': 'Non-Particip', '99': 'Ungraded'}})
    ...
    def formatWHDeductibleFactor(self, ws, boldFont):
        ...
        for col in range(1, ws.max_column + 1):
            ...
            if col <= 2:
                ...
                ws.column_dimensions[char].width = self.pixelsToInches(105) # Unique column width for column C
            elif col == 4:
                ws.column_dimensions[char].width = self.pixelsToInches(105) # Unique column width for column D
            else:
    ...
    def formatWHDeductiblePercentage(self, ws):
        for col in range(1, ws.max_column + 1):
            ...
            if col == 1:
                ...
                ws.column_dimensions[char].width = self.pixelsToInches(105)
            else:
                ws.column_dimensions[char].width = self.pixelsToInches(80)
            for row in range(4, ws.max_row + 1):
                cell = ws[char + str(row)]
                if col == 2: # Applying currency formatting to column B
                    cell.number_format = self.currencyFormat
    ...
    def buildAllPerilPage(self):
        ...
        AllPeril = ExcelSettings.Excel(state=self.state, programName='All Peril', nEffective=self.nEffective, rEffective=self.rEffective, companyList=companies)
        ...
        AllPeril.generateWorksheet('TR', 'AS, FS, H, O, R, S, W Table 3.C.1.a. State Territory Multiplier - All Perils', self.buildTerritoryMultiplier(), False, True)
        ...
        self.formatTerritoryMultiplier(AllPerilPages['TR'])
