...
class Hab:
    ...
    def buildHabPage(self):
        ...
        Hab = ExcelSettingsCurrent.Excel(state=self.state, programName='Habitational', nEffective=self.nEffective, rEffective=self.rEffective, companyList=companies)

        if 'NACO' in self.rateTables.keys():
            Hab.generateWorksheet('BRNACO', 'H Table 3.B.1. NW Assurance State Base Rates', self.buildBaseRates('NACO'), False, True)
        if 'NAFF' in self.rateTables.keys():
            Hab.generateWorksheet('BRNAFF', 'H Table 3.B.1. NW Affinity State Base Rates', self.buildBaseRates('NAFF'), False, True)
        ...
        if 'NICOF' in self.rateTables.keys():
            Hab.generateWorksheet('BRNICOF', 'H Table 3.B.1. NICOF State Base Rates', self.buildBaseRates('NICOF'), False, True)
        if 'NACO' in self.rateTables.keys():
            Hab.generateWorksheet('LANACO', 'H Table 3.C.5. Liability Charges for Related Additional Exposures', self.buildRelatedAddtExposures('NACO'), False, True)
        if 'NAFF' in self.rateTables.keys():
            Hab.generateWorksheet('LANAFF', 'H Table 3.C.5. Liability Charges for Related Additional Exposures', self.buildRelatedAddtExposures('NAFF'), False, True)
        if 'NGIC' in self.rateTables.keys():
        ...
        if 'NICOF' in self.rateTables.keys():
            Hab.generateWorksheet('LANICOF', 'H Table 3.C.5. Liability Charges for Related Additional Exposures', self.buildRelatedAddtExposures('NICOF'), False, True)
