...
class Auto:
    ...
    def buildBaseRates(self, company):
        ...
        filteredBuilingBaseRates = buildingBaseRates.query(f'Class_Code_Min == {self.autoProgramCode} & `Peril TypeCode` in {self.perils}').filter(items=['Peril TypeCode', 'BuildingBaseRate'])
        filteredBPPBaseRates = bppBaseRates.query(f'Class_Code_Min == {self.autoProgramCode} & `Peril TypeCode` in {self.perils}').filter(items=['Peril TypeCode', 'BPPBaseRate'])
    ...
    def buildTerritoryMultiplier(self, coverage):
        ...
        filteredTerritorialFactor = territorialFactor.query(f'Class_Code_Min == {self.autoProgramCode} & `Peril TypeCode` in {self.perils}').replace({'Peril TypeCode': self.perilsConversions}).rename(columns={'TerritoryCode': 'Territory'})
    ...
    def buildPDDeductibleAmount(self):
        ...
        return pdDeductibleAmount.query(f'ClassCode_Min == {self.autoProgramCode} & `Peril TypeCode` == "liability1"').rename(columns={'PDDeductibleAmount': 'P.D. Deductible Amount', 'PDDeductibleFactor': 'Factor'}). \
    ...
    def buildLiabilitySizeRisk(self):
        ...
        filteredLiabilitySizeRisk = liabilitySizeRisk.query(f'ClassCode_Min == {self.autoProgramCode} & `Peril TypeCode` == "liability1"').rename(columns={'ReceiptMin': 'Min', 'ReceiptMax': 'Max'}). \
                replace({'AutoServType': {'GRWG': 'General Repair With Gas', 'N/A': 'All Other Classes'}})
        return filteredLiabilitySizeRisk.pivot(index=['Min', 'Max'], columns='AutoServType', values='LiabilityFactor').reset_index(['Min', 'Max']).fillna({'Max': 'and over'})
    ...
    def buildAutoPage(self):
        ...
        AutoService = ExcelSettingsCurrent.Excel(state=self.state, programName='Auto Service', nEffective=self.nEffective, rEffective=self.rEffective, companyList=companies)

        fontName = AutoService.getFontName()
        fontSize = AutoService.getFontSize()

        if 'NACO' in self.rateTables.keys():
            AutoService.generateWorksheet('BRNACO', 'AS Table 3.B.1. NW Assurance State Base Rates', self.buildBaseRates('NACO'), False, True)
        if 'NAFF' in self.rateTables.keys():
            AutoService.generateWorksheet('BRNAFF', 'AS Table 3.B.1. NW Affinity State Base Rates', self.buildBaseRates('NAFF'), False, True)
        ...
        if 'NICOF' in self.rateTables.keys():
            AutoService.generateWorksheet('BRNICOF', 'AS Table 3.B.1. NICOF State Base Rates', self.buildBaseRates('NICOF'), False, True)
        AutoService.generateWorksheet('TRBG', 'AS Table 3.C.1.a. State Territory Multiplier - Building', self.buildTerritoryMultiplier('Building'), False, True)
        AutoService.generateWorksheet('TRPP', 'AS Table 3.C.1.a. State Territory Multiplier - BPP', self.buildTerritoryMultiplier('BPP'), False, True)
        AutoService.generateWorksheet('TRLB', 'AS Table 3.C.1.a. State Territory Multiplier - Liability', self.buildTerritoryMultiplier('Liability'), False, True)
        ...
        AutoService.generateWorksheet('OILN', 'AS Table 4.B.2. Optional Increased Limits - Employee Portable Tools and Equipment - Employee Limit is not equal to the Occurrence Limit', self.buildEmpPortToolsAndEquipment(False), False, True) 
        AutoService.generateWorksheet('OILE', 'AS Table 4.B.2. Optional Increased Limits - Employee Portable Tools and Equipment - Employee Limit is equal to the Occurrence Limit', self.buildEmpPortToolsAndEquipment(True), False, True)
        ...
        self.formatTerritoryMultiplier(AutoPages['TRBG'])
        self.formatTerritoryMultiplier(AutoPages['TRPP']) 
        self.formatTerritoryMultiplier(AutoPages['TRLB'])
