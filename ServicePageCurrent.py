...
class Service:
    ...
    def buildServicePage(self):
        ...
        Service = ExcelSettingsCurrent.Excel(state=self.state, programName='Service', nEffective=self.nEffective, rEffective=self.rEffective, companyList= companies)
        ...
        if 'NACO' in self.rateTables.keys():
            Service.generateWorksheet('BRNACO', 'S Table 3.B.1. NW Assurance State Base Rates', self.buildBaseRates('NACO'), False, True)
        if 'NAFF' in self.rateTables.keys():
            Service.generateWorksheet('BRNAFF', 'S Table 3.B.1. NW Affinity State Base Rates', self.buildBaseRates('NAFF'), False, True)
        ...
        if 'NICOF' in self.rateTables.keys():
            Service.generateWorksheet('BRNICOF', 'S Table 3.B.1. NICOF State Base Rates', self.buildBaseRates('NICOF'), False, True)
