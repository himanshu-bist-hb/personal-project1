# This module builds and formats the Habitational (Hab) State Page workbook (pre-2.0).
#
# Same idea as HabPage.py (BP-2.0) — see that file's module docstring — but
# transcribed from the root-level HabPageCurrent.py, which predates the
# Territory Definitions workbook (State Territory Multiplier tables are built
# straight from the ratebook's own BP7_Peril_TerritorialFactor table here,
# not a network file) and does not have the CA-only Habitability Exclusion
# sheet or the cat4/L-Products exclusions BP-2.0 added.
...
class Hab:
    ...
    # fallback. See [[bop-nesting-order]] — the root-level HabPageCurrent.py
    ...
    def buildBaseRates(self, company):
        ...
        filteredBuilingBaseRates = buildingBaseRates.query(f'Class_Code_Min == {self.habProgramCode} & `Peril TypeCode` in {self.perils}').filter(items=['Peril TypeCode', 'BuildingBaseRate'])
        filteredBPPBaseRates = bppBaseRates.query(f'Class_Code_Min == {self.habProgramCode} & `Peril TypeCode` in {self.perils}').filter(items=['Peril TypeCode', 'BPPBaseRate'])
        ...
        filteredPropertyBaseRates = propertyBaseRates.query(f'`Peril TypeCode` in {self.perils}'). \
    ...
    def buildNumOfUnitsFactor(self):
        ...
        filteredNumberOfUnits = numberOfUnits.query(f'`Peril TypeCode` == "liability1"').fillna({'NoOfUnits_Max': 0}). \
        ...
        return filteredNumberOfUnits.rename(columns={'LiabilityNoOfUnitsFactor': 'Factor'}).filter(items=['Units', 'Factor'])
    ...
    def buildNumOfStoriesFactor(self):
        ...
        filteredNumberOfStories = numberOfStories.query(f'`Peril TypeCode` == "liability1" & NoOfStories == NoOfStories').replace({'NoOfStories': {'4': '4 or More'}})
        return filteredNumberOfStories.rename(columns={'NoOfStories': 'Number of Stories', 'LiabilityNoOfStoriesFactor': 'Factor'}).filter(items=['Number of Stories', 'Factor'])
    ...
    def buildPDDeductibleAmount(self):
        ...
        return pdDeductibleAmount.query(f'ClassCode_Min == {self.habProgramCode} & `Peril TypeCode` == "liability1"').rename(columns={'PDDeductibleAmount': 'P.D. Deductible Amount', 'PDDeductibleFactor': 'Factor'}). \
    ...
    # Merges the "Number of Units" column of the D&O table into 3-row groups,
    # matching the root-level formatDirsOfficersLiabIns (pre-2.0 has no WA
    # 2-row special case — that was added in BP-2.0).
    ...
    def buildHabPage(self, progress_callback=None):
        ...
        sheetSpecs += [
            ('TRBG', 'H Table 3.C.1.a. State Territory Multiplier - Building', lambda: self.buildTerritoryMultiplier('Building'), False, True, 'PROGRAM_TR', None),
            ('TRPP', 'H Table 3.C.1.a. State Territory Multiplier - BPP', lambda: self.buildTerritoryMultiplier('BPP'), False, True, 'PROGRAM_TR', None),
            ('TRLB', 'H Table 3.C.1.a. State Territory Multiplier - Liability', lambda: self.buildTerritoryMultiplier('Liability'), False, True, 'PROGRAM_TR', None),
